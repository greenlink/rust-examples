...
fn main() -> Result<(), ImageDataErrors> {
    ...
    let (image_1, image_2) = stadardise_size(image_1, image_2);
...
fn find_image_path(path: String) -> Result<(DynamicImage, ImageFormat), ImageDataErrors> {
    match Reader::open(&path) {
        Ok(image_reader) => {
            if let Some(image_format) = image_reader.format() {
                match image_reader.decode() {
                    Ok(image) => return Ok((image, image_format)),
                    Err(e) => return Err(ImageDataErrors::UnableToDecodeImage(e))
                };
            } else {
...
fn get_samllest_dimensions(dim_1: (u32, u32), dim_2: (u32, u32)) -> (u32, u32) {
    let pix_1 = dim_1.0 * dim_1.1;
    let pix_2 = dim_2.0 * dim_2.1;
    return if pix_1 < pix_2 { dim_1 } else { dim_2 };
}

fn stadardise_size(image_1: DynamicImage, image_2: DynamicImage) -> (DynamicImage, DynamicImage) {
    let (width, height) = get_samllest_dimensions(image_1.dimensions(), image_2.dimensions());
    println!("widht: {}, height: {}\n", width, height);
